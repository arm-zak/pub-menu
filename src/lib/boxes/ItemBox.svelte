...
<div class='relative w-full lg:w-1/2 2xl:w-1/3'>
	...
	{/if}
	<div class:sold-out={!item.inStock}
			...
			<div class='grow flex flex-col'>
				...
				<div class='flex flex-row items-center '>
					{#if category.shownAttributes.includes("Country") && item.countryOrigin}
						<span class={"h-[15px] mr-2 w-[20px] fi fi-" + item.countryOrigin.toLowerCase()}></span>
					{/if}
		...
		<div class='w-full gap-x-2.5 h-1/4 flex flex-row mt-auto items-center'>
			...
			{#if category.shownAttributes.includes("House wine tags") && item.isHouseWine}
				<div class='group flex flex-row items-center justify-center bg-gray-400 rounded-full w-6 hover:px-2 hover:w-max h-6 shadow text-black'>
			...
			{#if category.shownAttributes.includes("Vegan tags") && item.vegan === "Yes"}
				<div class='group flex flex-row items-center justify-center bg-green-700 rounded-full w-6 hover:px-2 hover:w-max h-6 shadow text-black'>
			...
			{#if category.shownAttributes.includes("Vegetarian tags") && item.vegetarian === "Yes"}
				<div class='group flex flex-row items-center justify-center bg-green-400 rounded-full w-6 hover:px-2 hover:w-max h-6 shadow text-black'>
			...
			{#if category.shownAttributes.includes("Vegan tags") && item.vegan === "Optional"}
				<div class='group flex flex-row items-center justify-center bg-green-700/25 rounded-full w-6 hover:px-2 hover:w-max h-6 shadow text-black'>
					<i class="  fa-solid fa-leaf text-[13px]"></i>
			...
			{#if category.shownAttributes.includes("Vegetarian tags") && item.vegetarian === "Optional"}
				<div class='group flex flex-row items-center justify-center bg-green-400/25 rounded-full w-6 hover:px-2 hover:w-max h-6 shadow text-black'>
					<i class=" fa-solid fa-seedling text-[13px]"></i>
			...
			<div class='flex flex-row items-center gap-x-1'>
				{#if category.shownAttributes.includes("Untappd info") && (item.untappdUrl)}
					<a target='_blank' class='p-2 rounded bg-cover border border-slate-400'
