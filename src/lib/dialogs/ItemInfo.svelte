...
<div class='fixed top-0 left-0 w-full h-full z-30 backdrop-brightness-50' on:click={close}></div>

<div class=' md:translate-y-[-50%] md:translate-x-[-50%] overflow-auto fixed z-30 md:top-1/2 md:left-1/2 top-0 left-0 w-screen h-screen
 md:w-[calc(100vw_-_16rem)] md:max-w-6xl md:max-h-[calc(100vh_-_16rem)] px-3 pb-3 bg-white md:rounded-md md:h-fit' role='dialog' aria-modal='true' bind:this={modal}>
	...
	<div class='flex flex-col my-1 gap-y-1 items-center'>
		{#if $itemDialogItem.imageUrl}
			<img class='h-36 my-2' src={"https:"+$itemDialogItem.imageUrl + '?h=150&fm=webp'} alt='Item'>
		{/if}
