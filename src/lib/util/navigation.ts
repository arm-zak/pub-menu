...
export function updateActiveSubcategory() {
	const updatedActiveSubcategory = get(subcategories)
		.filter((subcategory) => subcategory.categoryId === get(activeCategory))
		.find((element) => element.inView)?.id;
	activeSubcategory.set(updatedActiveSubcategory || '');
	const element: HTMLElement | null = document.getElementById(
		'nav-subcategory-' + updatedActiveSubcategory
	);
	if (element) {
...
export function updateActiveCategory() {
	...
	const element: HTMLElement | null = document.getElementById(
		'nav-category-' + updatedActiveCategory
	);
	if (element) {
		element.scrollIntoView({ block: 'nearest' });
	}
