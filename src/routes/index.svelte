<script lang="ts">
	...
	categories.set(
		categoriesResponse
			.sort(sortDescending)
			.filter((element) => itemsResponse.some((obj) => obj.categoryId === element.id))
	);
	subcategories.set(
		subcategoriesResponse
			.sort(sortDescending)
			.filter((subcategory) => itemsResponse.some((obj) => obj.subcategoryId === subcategory.id))
	);
